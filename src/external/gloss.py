...
class Texture(object):
	...
	def draw(self, position = (0, 0), rotation = 0.0, origin = (0, 0), scale = 1, color = Color.WHITE):
		...
		Gloss.drawn = Gloss.drawn+1

	def drawClipped(self, position = (0, 0), color = Color.WHITE, clip=None):
		# never draw textures if picking
		if Gloss.picking:
			return
		...
		if clip:
			originx = 0
			originy = 0
			texwidth = clip[2] # -clip[0]
			texheight = clip[3] # -clip[1]
			
			texox = self.width_ratio*clip[0]/self.width
			texoy = 1+self.height_ratio*clip[1]/self.height
			texix = self.width_ratio*(clip[0]+clip[2])/self.width
			texiy = self.height_ratio*(clip[1]+clip[3])/self.height
		else:
			originx = 0
			originy = 0
			texwidth = self.width
			texheight = self.height
			
			texox = 0
			texoy = 0
			texix = self.width_ratio
			texiy = self.height_ratio
		...
		glTexCoord2f(texox, texiy); glVertex2f(originx, texheight + originy)
		glTexCoord2f(texix, texiy); glVertex2f(texwidth + originx, texheight + originy)
		glTexCoord2f(texox, texoy); glVertex2f(originx, originy)
		glTexCoord2f(texix, texoy); glVertex2f(texwidth + originx, originy)
		
		glEnd()
		
		Gloss.drawn = Gloss.drawn+1

	def drawRepeated(self, position = (0, 0), repeatx=1, repeaty=1, color = Color.WHITE):
		# never draw textures if picking
		if Gloss.picking:
			return
		...
		glBegin(GL_TRIANGLE_STRIP)
		glTexCoord2f(0, self.height_ratio); glVertex2f(-originx, texheight*repeaty - originy)
		glTexCoord2f(self.width_ratio, self.height_ratio); glVertex2f(texwidth*repeatx - originx, texheight*repeaty - originy)
		glTexCoord2f(0, 1); glVertex2f(-originx, -originy)
		glTexCoord2f(self.width_ratio, 1); glVertex2f(texwidth*repeatx - originx, -originy)
		glEnd()
		
		Gloss.drawn = Gloss.drawn+1
		
	def drawPie( self, position=(0,0), angleo=0, anglei=0, perc=50, color = Color.WHITE ):
		if Gloss.picking:
			return
		...
		originx = self.half_width
		originy = self.half_height
...
class SpriteFont(object):
	...
	def draw(self, text = "Hello, Gloss!", position = (0, 0), rotation = 0.0, scale = 1.0, color = Color.WHITE, letterspacing = 0, linespacing = 0):
		...
		for line in lines:
			for letter in line:
				if ord(letter) > self.endcharacter or ord(letter) < self.startcharacter:
					letter = "?"
