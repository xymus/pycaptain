...
class LocalConverter( Converter ):
    ...
    def getCommObjects( self, obj ):
        if isinstance( obj, Nebula ):
            return obj.clouds
        elif isinstance( obj, ShipWithTurrets ):
            return obj.getCommObjects()
     #   elif isinstance( obj, Bullet ):
     #       return obj.getCommObjects()
    #    elif isinstance( obj, HarvesterShip ):
    #        return obj.getCommObjects()
        else:
            # TODO, updatedAsCobjectAt in objects to avoid multiple processing per turn
            obj.type = obj.stats.img
            obj.selectRadius = obj.stats.maxRadius
            obj.uid = None
            obj.relation = None
            return [ obj ]
       ...
       ## objects + harvestable (-astres)
        if player.flagship:
          for obj in game.objects.getWithin( player.flagship, player.flagship.getRadarRange() ):
            if obj.alive:
             if (obj.xp + obj.stats.maxRadius/2 >= x0 \
              ...
              or obj.zp == -100:
                for cobj in self.getCommObjects( obj ): #obj.getCommObjects():
          ...
          #    and utils.distBetweenObjects( player.flagship, obj ) <= player.flagship.getRadarRange():
                cs = self.getCommObjects( obj ) #obj.getCommObjects()
