    ...
    def __init__(self, game, steps=None, name=None, description=None, year=0):
        ...
        steps = [
            Step( 
                ...
                texts = [ (0,"Look around your ship a bit."),
    (config.fps*3, "To move the camera, either use the arrow keys..."),
    (config.fps*6, "or click on the radar in the upper left corner.")] ),
            Step( 
                ...
                failure=lambda self, game: not self.player.flagship or not self.player.flagship.alive,
                texts = [ (0,"Good, now stick the camera to your ship."),
    (config.fps*3, "To do so, click on the center of the radar."),] ),
            Step( 
                ...
                failure=lambda self, game: not self.player.flagship or not self.player.flagship.alive,
                texts = [ (0,"Now move away from the orbital station."),
      (3*config.fps, "To move the ship, left-click on the destination and your crew will manoeuver towards there." ),
      (6*config.fps, "To give the order to stop, left-click on the ship." ) ] ),
            Step( 
                goal=lambda self, game: filter( lambda turret: turret.building==stats.T_SOLAR_0, self.player.flagship.turrets ),
                failure=lambda self, game: not self.player.flagship or not self.player.flagship.alive,
                texts = [ (0,"Let's prepare ourselves before going out to explore."),
          (3*config.fps, "Jumping uses a lot of energy, so first build 2 solar arrays on your ship." ),
          (6*config.fps, "To do so, left-click on one of the black circle at the right of the screen." ),           
          (9*config.fps, "Then select the solar array from the list." ) ] ),
            Step( 
                ...
                failure=lambda self, game: not self.player.flagship or not self.player.flagship.alive,
                texts = [ (0,"Well done, it will take a little while for it to be completed."),
                          (3*config.fps, "Begin building the second one right away." ),
                          (9*config.fps, "Notice the green bar in the top right corner..." ),
                          (12*config.fps, "it indicates the proportion of energy in your battery." ),
                          (15*config.fps, "The quantity is indicated in green text next to it.." ) ] ),
            Step( 
                ...
                failure=lambda self, game: not self.player.flagship or not self.player.flagship.alive,
                texts = [ (0,"Every ship benefits from solar energy the closer they are to a sun." ),
                    (3*config.fps,"A solar array will capture even more solar energy but will consume a little when in deep space." ) ] ),
            Step( 
                goal=lambda self, game: self.player.flagship.jumping,
                failure=lambda self, game: not self.player.flagship or not self.player.flagship.alive,
                texts = [ (0,"We still need to fill the battery before jumping."),
                          (3*config.fps, "Use the long-range sensor in fullscreen mode to see our destination." ),
                          (6*config.fps, "Left-click on the Radar button in the upper-left corner." ),
                          (12*config.fps, "Notice your poition over Earth. It will be useful to come back." ),
                          (15*config.fps, "Mars is the 4th planet from the sun." ),
                          (18*config.fps, "You can see it on the right of the asteroid field down left from you."),
                          (21*config.fps, "To execute the jump, left-click on the blue button at the top of the screen" ),
                          (24*config.fps, "the left-click on the destination, the planet Mars." ), ] ),
            Step( 
                ...
                texts = [ ( 0, "Know that you can change the destination of the jump while it charges." ),
                          (3*config.fps, "Go back to normal view by clicking on the radar button again." ), ] ),
            Step( 
                goal=lambda self, game: utils.distLowerThan( self.ennemyPosition, self.player.flagship.pos, 500 ),
                failure=lambda self, game: not self.player.flagship or not self.player.flagship.alive,
                texts = [ ( config.fps, "Now head for the asteroid field." ),
                    ( 4*config.fps, "Reports are that there is a anormal rotating asteroid towards the bottom of the field." ),
                    ( 7*config.fps, "Investigate it." ) ] ),
            Step( 
                goal=lambda self, game: utils.distLowerThan( self.ennemyPosition, self.player.flagship.pos, 200 ),
                failure=lambda self, game: not self.player.flagship or not self.player.flagship.alive,
                texts = [ ( 0, "It may be dangerous but get closer to gather data." ) ] ),
            Step( 
                goal=lambda self, game: self.player.flagship.jumping,
                failure=lambda self, game: not self.player.flagship or not self.player.flagship.alive,
                texts = [ ( 0, "What ever it is. It is hostile!" ),
                    ( 1*config.fps, "Enough data collected." ),
                    ( 2*config.fps, "Jump away now!" ) ] ),
            Step( 
                goal=lambda self, game: not self.player.flagship.jumping,
                failure=lambda self, game: not self.player.flagship or not self.player.flagship.alive,
                texts = [ ( 0, "Don't worry, your shield and hull should be able to handle the hits for a little longer." ) ] ),
            Step( 
                goal=lambda self, game: utils.distLowerThanObjects( self.player.flagship, self.orbitalbase, 2000 ),
                failure=lambda self, game: not self.player.flagship or not self.player.flagship.alive,
                texts = [ ( 1, "Head back to Earth." ) ] ),
            Step( 
                goal=lambda self, game: utils.distLowerThanObjects( self.player.flagship, self.orbitalbase, 100 ),
                failure=lambda self, game: not self.player.flagship or not self.player.flagship.alive,
                texts = [ (0, "Return to the orbital station."),
                    ( 3*config.fps, "Collected data will be useful if they ever try to get closer to Earth." ) ] ),
                ]
